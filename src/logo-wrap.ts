...
import { VOLKBUSTER_TRIM, VOLKBUSTER_CREAM, VOLKBUSTER_BLUE, VOLKBUSTER_INK } from './logo-spec';
...
// spec still carries the untouched VolkBuster defaults; any user recolor is
// honored verbatim.
const WRAP_PRINT_LETTER = '#b5731f'; // rust-brass lettering ink
const WRAP_PRINT_BODY = VOLKBUSTER_INK; // deeper emerald for the printed panels
...
function wrapPrintColors(spec: LogoSpec): { body: string; letter: string; stripe: string } {
  const isDefaultInk = spec.textColor.toLowerCase() === VOLKBUSTER_CREAM;
  return {
    body: spec.bodyColor.toLowerCase() === VOLKBUSTER_BLUE ? WRAP_PRINT_BODY : spec.bodyColor,
    letter: isDefaultInk ? WRAP_PRINT_LETTER : spec.textColor,
    stripe: spec.borderColor.toLowerCase() === VOLKBUSTER_TRIM ? WRAP_PRINT_LETTER : spec.borderColor,
  };
}
